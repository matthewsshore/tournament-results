...
def playerStandings():
    ...
    standings = [[int(row[0]), str(row[1]), int(row[2]), int(row[3])] for row in c.fetchall()]
    conn.close()
    return standings
...
def swissPairings():
    ...
    for x in range(0,matches_needed):
        match_array.append([standings[y][0],standings[y][1],standings[y+1][0],standings[y+1][1]])
        y = y + 2
        #id_array.append(standings[x][0])
        #name_array.append(standings[x][1])
    conn.close()
    print match_array
    #print name_array
    #print id_array
